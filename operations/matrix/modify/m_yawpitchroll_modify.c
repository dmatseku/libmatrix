								 ...
								 const float pitch, const float roll)
{
	...
	matrix->mat[2] = 2 * (vector.w * vector.y + vector.x * vector.z);
	matrix->mat[4] = 2 * (vector.x * vector.y + vector.w * vector.z);
	matrix->mat[5] = 1 - 2 * (vector.x * vector.x + vector.z * vector.z);
	matrix->mat[6] = 2 * (vector.y * vector.z - vector.w * vector.x);
	matrix->mat[8] = 2 * (vector.x * vector.z - vector.w * vector.y);
	matrix->mat[9] = 2 * (vector.w * vector.x + vector.y * vector.z);
